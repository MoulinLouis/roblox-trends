...
const settingsSchema = z.object({
  ...
  collection: z.object({ intervalMinutes: z.number().int().min(15).max(1440), country: z.string().min(2).max(8), device: z.string().min(2).max(30), charts: z.array(z.string().min(1)).min(1), rolimonsEnabled: z.boolean(), rolimonsCandidates: z.number().int().min(0).max(100) }),
