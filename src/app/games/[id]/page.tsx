...
export default async function GameDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <div className="content"><div className="detail-hero"><GameThumbnail name={item.game.normalizedTitle} url={item.game.thumbnailUrl} large /><div className="detail-hero-copy"><p className="eyebrow">Game signal · Universe {item.game.universeId}</p><h1>{item.game.normalizedTitle}</h1><p className="page-subtitle">By {item.game.creatorName} · Created on Roblox {formatDate(item.game.createdAt)} · First tracked {formatDate(item.game.firstSeenAt)}</p></div><ScoreRing score={item.analysis?.momentumScore ?? 0} large /><div className="detail-actions"><a className="button" href={`https://www.roblox.com/games/${item.game.rootPlaceId}`} target="_blank" rel="noreferrer">Open Roblox <ExternalLink size={14} /></a></div></div>
    ...
    <div className="dashboard-layout section"><div className="card card-pad"><h2>Momentum score breakdown</h2><div className="breakdown">{metrics?.momentum.breakdown.map((part) => <div className="breakdown-row" key={part.key}><div className="breakdown-name"><strong>{part.label}</strong><span>{part.explanation}</span></div><div className="bar"><span style={{ width: `${part.normalized}%` }} /></div><span className="breakdown-points">{part.points.toFixed(1)} pts</span></div>)}</div></div><div className="card card-pad"><h2>Metadata</h2><div className="metadata"><div className="metadata-row"><span>Created</span><strong>{item.game.createdAt.toLocaleDateString("en")}</strong></div><div className="metadata-row"><span>Last updated</span><strong>{item.game.updatedAt.toLocaleDateString("en")}</strong></div><div className="metadata-row"><span>Visits</span><strong>{formatCompact(point?.visits ?? 0)}</strong></div><div className="metadata-row"><span>Favorites</span><strong>{formatCompact(point?.favorites ?? 0)}</strong></div><div className="metadata-row"><span>Approval</span><strong>{metrics?.approvalRate ? `${metrics.approvalRate.toFixed(1)}%` : "Not collected yet"}</strong></div><div className="metadata-row"><span>New-like efficiency</span><strong>{(metrics?.likesPerThousandVisits24h ?? 0).toFixed(1)} / 1k visits</strong></div><div className="metadata-row"><span>Genre</span><strong>{item.game.genre || "Unclassified"}</strong></div><div className="metadata-row"><span>Collection source</span><strong>{point?.chart || "Unknown"}</strong></div></div></div></div>
    {item.metadataHistory.length > 1 ? <section className="section card card-pad"><h2>Recent metadata changes</h2><p className="page-subtitle">Title and update changes are retained so update-driven spikes remain visible after promotional markers are removed.</p><div className="list">{item.metadataHistory.slice(-5).reverse().map((entry) => <div className="list-row" key={entry.id}><div className="list-row-main"><strong>{entry.normalizedTitle}</strong><span>Observed {formatDate(entry.observedAt)} · game updated {formatDate(entry.gameUpdatedAt)}</span></div></div>)}</div></section> : null}
...
function signalWindow(hours: number): string {
  ...
  return `${Math.max(1, Math.round(hours))}h provisional`;
}
