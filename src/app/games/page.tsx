...
export default async function GamesPage({ searchParams }: { searchParams: Promise<SearchParams> }) {
  ...
  const items = dataset
    .filter((item) => {
      ...
      return age >= minAge && age <= maxAge && ccu >= minCcu && growth >= minGrowth && (!filters.tag || item.tags.some((tag) => tag.tag === filters.tag)) && (!filters.stage || gameStages.has(filters.stage as TrendStage));
    })
    .sort((a, b) => compareGames(a, b, filters.sort || "momentum"));
...
function compareGames(a: Awaited<ReturnType<typeof loadApplicationData>>["dataset"][number], b: Awaited<ReturnType<typeof loadApplicationData>>["dataset"][number], sort: string): number {
